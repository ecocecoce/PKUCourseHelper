...
void MainWindow::press_table(int x, int y){
    if (plan_now == NULL){
        ui->TitleLabel->setText("<html><head/><body><p align=""center""><span style="" font-size:22pt;"">果咩纳塞(*^▽^*)</span></p></body></html>");
        ui->TitleLabel->setStyleSheet("color:blue;background-color:pink");
    }
    else{//只显示冲突信息
        if (table_inform[x][y].size() > 1){
            ui->InformNameLabel->setText("课程名");
        ...
        else if (table_inform[x][y].empty()){
            ui->TitleLabel->setText("<html><head/><body><p align=""center""><span style="" font-size:22pt;"">果咩纳塞o(╥﹏╥)o</span></p></body></html>");
            ui->TitleLabel->setStyleSheet("color:blue;background-color:pink");
        }
    }
}
...
void MainWindow::release_table(){
    ui->TitleLabel->setText("<html><head/><body><p align=""center""><span style="" font-size:22pt;"">欢迎来到PKU选课助手！</span></p></body></html>");
    ui->TitleLabel->setStyleSheet("");
    if (plan_now == NULL){
        return;
    }
    else{
        ui->InformNameLabel->setText("计划名");
        for (int k = 0;k < (int)course_inform[0].size();k++){
            for (int l = 0;l < 4;l++)
                delete course_inform[l][k];
        }
        ...
        int _point = 0;
        int _time = 0;
        for (int k = 0;k < (int)plan_now->courses.size();k++){
            _priority += plan_now->courses[k]->priority;
            _point += plan_now->courses[k]->point;
            _time += plan_now->courses[k]->total_time;
        }
        QLabel* name = new QLabel(this);
        QLabel* pri = new QLabel(this);
        QLabel* point = new QLabel(this);
        QLabel* time = new QLabel(this);
        name->setText(plan_now->name);
        pri->setNum(_priority);
        point->setNum(_point);
        time->setNum(_time);
        name->setAlignment(Qt::AlignCenter);
        pri->setAlignment(Qt::AlignCenter);
        ...
        for (int i = 1;i <= 7;i++){
            for (int j = 1;j <= 12;j++){
                if (table_inform[i][j].empty()){
                    ...
                    switch(table_inform[i][j][0]->priority){
                    case(0):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 255, 0)");
                        break;
                    case(1):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 255, 64)");
                        break;
                    case(2):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 255, 128)");
                        break;
                    case(3):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 255, 192)");
                        break;
                    case(4):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 255, 255)");
                        break;
                    case(5):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 192, 255)");
                        break;
                    case(6):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 128, 255)");
                        break;
                    case(7):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 80, 255)");
                        break;
                    case(8):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 40, 255)");
                        break;
                    case(9):
                        table_show[i][j]->setStyleSheet("color:white;background-color:rgb(0, 0, 255)");
                        break;
                    }
                    ...
                    table_show[i][j]->setText("冲突");
                    table_show[i][j]->setStyleSheet("color:black;background-color:red");
                    table_show[i][j]->setVisible(true);
                }
            }
        }
...
void MainWindow::click_plan(Plan* plan){
    ...
    if (plan_now == NULL){  //显示plan
        ...
        for (int k = 0;k < (int)plan->courses.size();k++){
            ...
            _priority += plan->courses[k]->priority;
            _point += plan->courses[k]->point;
            _time += plan->courses[k]->total_time;
        }
...
void MainWindow::press_plan(Plan* plan){
    if (delete_flag){
        plan_now = plan;
        renew_table();
    }
...
void MainWindow::release_plan(Plan* plan){
    if (delete_flag){
        plan_now = NULL;
        renew_table();
    }
...
void MainWindow::click_course(Course* course){
    ...
    if (plan_now == NULL){
        ...
        course_inform[1][0]->setNum(_priority);
        course_inform[2][0]->setNum(_point);
        course_inform[3][0]->setNum(_time);
    }
}
...
void MainWindow::on_AddPlan_clicked()
{
    ...
    connect(btn, &QPushButton::pressed, this, [=](){this->press_plan(plan);});
    connect(btn, &QPushButton::released, this, [=](){this->release_plan(plan);});
